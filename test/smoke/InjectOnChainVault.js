...
describe("InjectOnChainVault", function () {
    ...
    const calculateFreeFunds = async (oldVault, blockNumber) => {
        const blockTimestamp = ethers.BigNumber.from((await ethers.provider.getBlock(blockNumber)).timestamp);
        const lastReport = await oldVault.lastReport();
        const lockedProfitDegradation = await oldVault.lockedProfitDegradation();
        const lockedFundsRatio = blockTimestamp.sub(lastReport).mul(lockedProfitDegradation);
        ...
        if (lockedFundsRatio.lt(DEGRADATION_COEFFICIENT)) {
            lockedProfit = lockedProfit.sub(lockedFundsRatio.mul(lockedProfit).div(DEGRADATION_COEFFICIENT));
        } else {
