...
describe("AuraWETHStrategy", function () {
  ...
  it("should sweep", async function () {
    ...
    await expect(
      strategy.connect(deployer)["sweep(address)"](TOKENS.WETH_AURA_BPT.address)
    ).to.be.revertedWith("!protected");
  ...
  it("should migrate", async function () {
    ...
    expect(Number(await newStrategy.balanceOfUnstakedBpt())).to.be.greaterThan(
      0
    );
    ...
    expect(Number(await strategy.balanceOfAuraBpt())).to.be.equal(0);
    expect(Number(await newStrategy.balanceOfAuraBpt())).to.be.greaterThan(0);
  });
