...
describe("GMXStrategy", function () {
  ...
  it("should migrate", async function () {
    ...
    expect(Number(await strategy.balanceOfUnstakedEsGmx())).to.be.not.lessThan(
      Number(unstakedEsGmxBalance)
    );
    expect(Number(await newStrategy.balanceOfStakedEsGmx())).to.be.not.lessThan(
      Number(stakedEsGmxBalance)
    );
  ...
  it("should accrue some rewards after some time", async function () {
    ...
    expect(Number(await strategy.balanceOfWethRewards())).to.be.greaterThan(0);
  });
  ...
  it("should stake esGMX", async function () {
    ...
    expect(Number(await esGmxToken.balanceOf(strategy.address))).to.be.equal(
