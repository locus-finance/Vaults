...
describe("RocketAuraStrategy", function () {
  ...
  it("should scale decimals", async function () {
    ...
    expect(
      await testScaler.scaleDecimals(ethers.utils.parseEther("1"), usdt, bal)
    ).to.be.equal(BigNumber.from("1000000000000000000000000000000"));

    expect(
      await testScaler.scaleDecimals(ethers.utils.parseEther("1"), bal, usdt)
    ).to.be.equal(BigNumber.from("1000000"));

    expect(
      await testScaler.scaleDecimals(ethers.utils.parseEther("1"), bal, dai)
  ...
  it("should not get aura rewards after inflation protection time", async function () {
    ...
    const TestAuraMath = await hre.ethers.getContractFactory("TestAuraMath");
  ...
  it("should change AURA PID and AURA rewards", async function () {
    ...
    expect((await strategy.auraBRethStable()).toLocaleLowerCase()).to.be.equal(
      auraBRethStable.toLocaleLowerCase()
    );
    await expect(
      strategy
        .connect(whale)
    ...
    await strategy
      .connect(deployer)
      ["setAuraBRethStable(address)"](constants.AddressZero);
    expect(await strategy.auraBRethStable()).to.be.equal(constants.AddressZero);
  });
});
