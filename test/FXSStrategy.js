...
describe("FXSStrategy", function () {
  ...
  it("should get reasonable prices from oracle", async function () {
    ...
    expect(Number(await strategy.curveLPToWant(oneUnit))).to.be.greaterThan(0);
  ...
  it("should sweep", async function () {
    ...
    await expect(
      strategy.connect(deployer)["sweep(address)"](TOKENS.CURVE_FXS_LP.address)
    ).to.be.revertedWith("!protected");
  ...
  it("should migrate", async function () {
    ...
    expect(Number(await newStrategy.balanceOfCurveLPStaked())).to.be.equal(0);
    expect(Number(await want.balanceOf(newStrategy.address))).to.be.equal(0);
    expect(Number(await strategy.balanceOfCurveLPUnstaked())).to.be.equal(0);
    expect(Number(await newStrategy.balanceOfCurveLPUnstaked())).to.be.equal(
