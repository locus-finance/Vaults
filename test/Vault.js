...
describe("Vault", function () {
  async function deployVaultAndSetVariables() {
    const [deployer, whale, governance, treasury, user1, user2, user3] =
    ...
    await token
      .connect(whale)
      .transfer(user3.address, ethers.utils.parseEther("100"));
  ...
  it("should set the treasury correctly", async function () {
    const { vault, token, treasury } = await loadFixture(
      deployVaultAndSetVariables
    );
    expect(await vault.rewards()).to.equal(treasury.address);
  });

  it("should receive deposit", async function () {
    const { vault, whale, token } = await loadFixture(
      deployVaultAndSetVariables
    );
  ...
  it("should not receive deposit over limit", async function () {
    ...
    expect(await vault.availableDepositLimit()).to.equal(
      ethers.utils.parseEther("0.01")
    );
  });

  it("should not receive deposit over limit when has active strategy", async function () {
    const { vault, whale, deployer } = await loadFixture(
      deployVaultAndSetVariables
    );
    const TestStrategy = await ethers.getContractFactory("TestStrategy");
    const strategy = await TestStrategy.connect(deployer).deploy(vault.address);
  ...
  it("should not receive any deposit when deposit limit is zero", async function () {
    ...
    const amount = 1;
    await token.connect(whale).approve(vault.address, amount);
    await expect(vault.connect(whale)["deposit(uint256)"](amount)).to.be
      .reverted;
  ...
  it("should not withdraw when no idle and withdrawal queue is empty", async function () {
    const { vault, whale, deployer, token } = await loadFixture(
      deployVaultAndSetVariables
    );
    ...
    ).to.changeTokenBalances(token, [vault, whale, strategy], [0, 0, 0]);

    await expect(vault.connect(whale)["withdraw(uint256)"](amount))
  ...
  it("should set governance", async function () {
    const { vault, deployer, governance } = await loadFixture(
      deployVaultAndSetVariables
    );
  ...
  it("should be able to withdraw part of assets when strategy is active but not in withdrawal queue", async function () {
    const { vault, deployer, whale, token } = await loadFixture(
      deployVaultAndSetVariables
    );
    ...
    expect(await vault.maxAvailableShares()).to.equal(
  ...
  it("should be able to withdraw all assets when strategy is active and in withdrawal queue", async function () {
    const { vault, whale, token, strategy } = await loadFixture(
  ...
  it("should give strategy credit", async function () {
    ...
    await expect(() => strategy.harvest()).to.changeTokenBalances(
      token,
      [strategy, vault],
  ...
  it("should handle strategy profit", async function () {
    ...
    await token
      .connect(whale)
      .transfer(strategy.address, ethers.utils.parseEther("0.23"));
  ...
  it("should handle strategy loss", async function () {
    const { vault, deployer, whale, token, strategy } = await loadFixture(
      deployVaultAndStrategy
    );
    ...
    await strategy
      .connect(deployer)
      ._takeFunds(ethers.utils.parseEther("0.23"));
    await strategy.harvest();
    ({ totalLoss } = await vault.strategies(strategy.address));
  ...
  it("should give the same price to all depositors when strategy closing", async function () {
    const { vault, deployer, token, whale, user1, user2, user3 } =
      await loadFixture(deployVaultAndSetVariables);
    ...
    expect(await vault.balanceOf(user3.address)).to.equal(
      ethers.utils.parseEther("0.5")
    );
    expect(await vault.pricePerShare()).to.equal(ethers.utils.parseEther("2"));
