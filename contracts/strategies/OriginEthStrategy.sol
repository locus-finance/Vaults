    ...
    OwnableUpgradeable
{
    ...
    address internal constant CRV_WETH_UNI_POOL =
    ...
    uint256 private constant PID = 174;
        ...
        returns (uint256 _wants)
    {
        _wants =
            balanceOfWant() +
            LPToWant(balanceOfLPStaked()) +
            LPToWant(balanceOfLPUnstaked()) +
            CrvToWant(getRewards());
    }
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        if (_excessWant > 0) {
            ...
            _unwrap(_excessWant);
            uint256 minMintAmount = (wantToLp(_excessWant) * slippage) /
                SLIPPAGE_BASE;
            ICurve(LP_TOKEN).add_liquidity{value: _excessWant}(
                amounts,
                minMintAmount,
                address(this)
            );
        ...
        if (balanceOfLPUnstaked() > 0) {
            bool suc = IConvexDeposit(CONVEX).deposit(
                PID,
                balanceOfLPUnstaked(),
                true
            );
    ...
    function CrvToWant(
        ...
        amountOut = smthToSmth(CRV_WETH_UNI_POOL, REWARD_TOKEN, WETH, amountIn);
    }

    function _exitPosition(uint256 _stakedLpTokens) internal {
        _claimAndSellRewards();
        bool suc = IConvexRewards(REWARDS).withdrawAndUnwrap(
            _stakedLpTokens,
            false
        );
        require(suc == true, "withdraw unsuccessful");
        uint256 minTokenToRec = (LPToWant(balanceOfLPUnstaked()) * slippage) /
            SLIPPAGE_BASE;
        ILPToken(LP_TOKEN).remove_liquidity_one_coin(
            balanceOfLPUnstaked(),
            0,
            minTokenToRec
        );
    ...
    function _claimAndSellRewards() internal {
        ...
        params.amountOutMinimum =
            (CrvToWant(IERC20(REWARD_TOKEN).balanceOf(address(this))) *
                slippage) /
            SLIPPAGE_BASE;
